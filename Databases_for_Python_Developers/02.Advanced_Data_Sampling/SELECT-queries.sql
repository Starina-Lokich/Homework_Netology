...
--Название и продолжительность самого длительного трека (исправлено)
...
LEFT JOIN Исполнитель_Альбом иа ON иа.Исполнитель_id = Исполнитель.id
LEFT JOIN Альбом ON иа.Альбом_id = Альбом.id
WHERE Альбом.Год_выпуска NOT IN  
	(SELECT Альбом.Год_выпуска  FROM Исполнитель
	LEFT JOIN Исполнитель_Альбом иа ON иа.Исполнитель_id = Исполнитель.id
	LEFT JOIN Альбом ON иа.Альбом_id = Альбом.id
	WHERE Альбом.Год_выпуска BETWEEN '2020-01-01' AND '2020-12-31');

--Названия сборников, в которых присутствует конкретный исполнитель (выберите его сами) (исправлено)
